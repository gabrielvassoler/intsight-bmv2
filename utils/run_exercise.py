...
class ExerciseTopo(Topo):
    ...
    def __init__(self, hosts, switches, links, log_dir, **opts):
        ...
        new_link_sort_key = lambda x: (int(x['node1'][1:]), int(x['node2'][1:]))
        # Links must be added in a sorted order so bmv2 port numbers are predictable
        host_links.sort(key=new_link_sort_key)
        switch_links.sort(key=new_link_sort_key)
...
class ExerciseRunner:
    ...
    def __init__(self, topo_file, log_dir, pcap_dir, switch_json,
                 bmv2_exe='simple_switch', quiet=False, username=None):
        ...
        self.capture_traffic = topo['capture_traffic']
        self.run_workload = topo['run_workload']
        if self.run_workload:
            self.workload_file = topo['workload_file']
        else:
    ...
    def run_exercise(self):
        ...
        # intsight receiver
        print 'Starting IntSight receiver'
        with open('/tmp/intsight_flag', 'w') as f:
            f.write('0')

        print('Using username "{}" for log files'.format(self.username))
        cmd = ['sudo', 'python', 'report-receiver.py', str(len(self.switches) - 1), str(self.username)]
        is_receiver = subprocess.Popen(cmd)
        print 'process received pid: {}'.format(is_receiver.pid)
    ...
    def create_network(self):
        ...
        if self.capture_traffic:
            switchClass = configureP4Switch(
                ...
                enable_debugger=False,  # no debugging possible in p4eva
                log_console=False,  # no logging possible in p4eva
